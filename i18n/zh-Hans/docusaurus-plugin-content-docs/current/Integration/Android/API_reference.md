...
### prepare
准备引擎所需的资源并进行鉴权。首次调用时会下载音色模型文件，耗时较长。

### getVoiceList
获取可用的音色列表。只有在引擎准备完成后，才会返回可用的音色列表。
...
**返回值：** `int`，被删除的文件数量。  
**注意：** 引擎会自动清理不需要的文件，请勿在引擎的文件目录中存放其他文件。
...
**注意：** 只有在引擎准备完成后才能设置成功。无论当前是否正在进行语音变换，都可以设置音色。例如，在语音变换进行中，设置成功后声音会立即切换为新的音色。
...
| sampleRate | int | 16000 | 输入采样率 |
| samplesPerCall | int | | 每次音频数据回调的采样大小。默认由引擎计算：samplesPerCall = outputSampleRate / 100 * 16 * 2 |
| mToken | String | null | 鉴权 Token，由开发者获取 |
| debugHelper | TransformDebugHelper | null | 计算辅助类，用于统计变声过程中的性能数据，仅在 transformDebug 为 true 时生效 |
| muteOnFail | boolean | true | 变声失败时是否静音 |
| debug | boolean | false | 是否打印引擎运行日志 |
| transformDebug | boolean | false | 是否打印变声处理过程日志，开启后会大幅增加日志输出 |
...
开启变声日志。语音变换属于高频操作，开启后日志刷新速度会非常快。

### enableTransformLog(TransformDebugHelper)（可选）
效果与 transformLog() 相同，同时 helper 会记录语音变换过程中的各类耗时数据。

### token
鉴权 Token，由服务端下发。1.x 版本必填，2.x 版本可选。

### sampleRate（可选）
采样率，默认值为 16000。常见可选值包括 16000、24000、48000。

### engineCallback(callback)（可选）
引擎回调。

| 参数名 | 参数类型 | 说明 |
| :--- | :--- | :--- |
| callback | DubbingCallback 接口 | /** 下载进度 * @param percent 当前下载文件进度，0–100，100 表示当前文件下载完成 * @param index 当前下载文件索引，从 1 开始 * @param count 需要下载的文件总数 */ fun onDownload(percent: Int , index: Int, count: Int) |
| | | /** 事件回调 * @param action 事件类型 * @param code 事件结果 * @param msg 消息 */ fun onActionResult(action: DubbingAction, code: DubbingEngineCode, msg: String? = null ) |
...
### samplesPerCall
每次音频数据回调的采样大小。  
每一帧的数据长度必须是 10 毫秒的整数倍。  
例如需要 10 毫秒的数据，16 bit，单声道，采样率为 16000：
